...
export function handleFetch(event: FetchEvent) {
  ...
  // `/auth/sign_out/callback` is this fork's Keycloak OIDC post-logout redirect
  // (see keycloak_social_post_logout_redirect_uri in yoush-infra). Both paths must
  // clear the cache: upstream's own sign-out still exists, and the OIDC flow lands
  // on the callback instead. This was a one-line replacement in the old sw.js,
  // which upstream deleted in "Remove PWA plugin (#39250)"; adding rather than
  // replacing keeps upstream's tests passing and shrinks the next merge conflict.
  if (
    url.pathname === '/auth/sign_out' ||
    url.pathname === '/auth/sign_out/callback'
  ) {
